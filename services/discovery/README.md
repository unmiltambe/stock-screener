...
Runs as an EventBridge-scheduled Lambda (target: daily), reusing `services/core` for
scoring and `services/adapters` for data. Writes a ranked `UNIVERSE#<asOf>` snapshot
...
([P5](../../docs/constitution.md)); don't fork scoring logic — reuse `services/core`.
