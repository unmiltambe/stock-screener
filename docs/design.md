...
`userId` comes from the verified token claim only (FR-1.5, NFR-3.1).

```
GET    /health                          → liveness, no auth

GET    /watchlists                      → user's lists (names + counts)
POST   /watchlists                      → create  { name }
GET    /watchlists/{name}               → list + each ticker scored (FR-3.1)
PATCH  /watchlists/{name}               → rename  { newName }
DELETE /watchlists/{name}               → delete
PUT    /watchlists/{name}/tickers/{sym} → add ticker
DELETE /watchlists/{name}/tickers/{sym} → remove ticker

GET    /leaderboard                     → aggregated ranked views (FR-5.1)

GET    /tickers/{sym}/chart?years=1     → price + SMA-50 + SMA-200 (FR-3.6)
GET    /scores?tickers=A,B,C            → scores for an arbitrary set (cache-first)

GET    /screen?minFund=60&maxPeg=2&sector=Tech&sort=combined  → screen the
                                          universe from precomputed rankings (FR-6.2/6.3)
GET    /suggestions                      → top universe names not in user's lists (FR-6.4)

GET    /comments/{sym}                  → user's comment
PUT    /comments/{sym}                  → upsert  { text }
```
...
| Entity | PK | SK | Notes |
|--------|----|----|-------|
| Watchlist | `USER#<sub>` | `WL#<name>` | attr `tickers: [..]` |
| Comment | `USER#<sub>` | `CMT#<sym>` | attr `text` |
